...
contract Lender {
    ...
    uint public platformFeeBase = 1000;
    ...
    function newPledge(address payable _borrower, uint _increment) external {
        ...
        require(_increment >= platformFeeBase, 'must pledge at least the minimum amount');
    ...
    function newLoan(address payable _borrower) internal {
        ...
        require(Coin.balanceOf(address(this)) - piggybank >= _request.amount, 'lender contract balance not enough');
    ...
    function closeRequest(address payable _borrower) external {
        ...
        Request storage _request = requests[_borrower];
        require(Coin.balanceOf(address(this)) - piggybank >= _request.pledge, 'lender contract balance not enough');
    ...
    function _transitionToLoanState(LoanState to, address payable _borrower, uint _repayment) internal {
        ...
        if(to == LoanState.CLOSED || to == LoanState.OVERDUECLOSED){
            ...
            require(_loan.amount.add(_loan.interest).add(_loan.overdueFee) <= Coin.balanceOf(address(this)) - piggybank, 'lender contract balance not enough');
            for(uint  i; i < _loan.lenders.length; i++) {
                uint _totalRepayment = _loan.lenderPledges[_loan.lenders[i]].add(_loan.lenderInterests[_loan.lenders[i]]).add(_loan.lenderOverdueFees[_loan.lenders[i]]);
                uint _platformFee = _loan.lenderPledges[_loan.lenders[i]].mul(platformFee).div(platformFeeBase);
                uint _lenderRepayment = _totalRepayment.sub(_platformFee);
                Coin.transfer(_loan.lenders[i], _lenderRepayment);
                piggybank = piggybank.add(_platformFee);
            }
            _transitionToRequestState(RequestState.CLOSED, _borrower);
